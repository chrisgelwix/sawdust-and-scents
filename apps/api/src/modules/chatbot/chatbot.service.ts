...
export class ChatbotService {
  ...
  async processMessage(
    ...
    if (
      ...
      (input.includes('scent') || input.includes('smell'))
    ) {
      ...
      if (requestedScent) {
        const products = await this.productsService.findByAttribute(
          'scent',
          requestedScent
        );
        if (products.length > 0) {
          return {
            reply: `I found ${products.length} ${requestedScent} scented candles! Our ${products[0].name} is a popular choice.`,
          };
        }
      }
      return {
        reply:
          'We have several scented candles available! Are you looking for something woody like Sandalwood or fresh like Pine?',
      };
    }
    ...
    if (input.includes('candle')) {
      return {
        reply:
          'We have a variety of hand-poured candles. Most are approximately 4 inches wide and 5 inches tall, but sizes vary by collection.',
      };
    }

    return {
      reply:
        "I'm Rowan! I can help you with order status or product questions.",
    };
  }
